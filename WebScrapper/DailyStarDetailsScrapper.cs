...
{
    public class DailyStarDetailsScrapper
    {
        private readonly HttpClient _client;
        private readonly INewsContentService _newsContentService;

        public DailyStarDetailsScrapper(INewsContentService newsContentService)
        {
        ...
        public List<string> ExtractCategoryLink(string htmlContent)
        {
            HtmlDocument document = new HtmlDocument();
            document.LoadHtml(htmlContent);

            
            //var categoryNodes = document.DocumentNode.SelectNodes("//div[@class='columns main-menu-wrapper']//ul//li//a[@href]");
            var categoryNodes = document.DocumentNode.SelectNodes("//div[@class='columns main-menu-wrapper']//ul//li//a[text()='Sports']");
            //var categoryNodes = document.DocumentNode.SelectNodes("//nav//ul//li//a[text()='World']");
            var categoryLinks = new HashSet<string>();
            ...
            if (categoryNodes != null)
            {
                foreach (var node in categoryNodes)
                {
                    ...
                    if (!string.IsNullOrEmpty(href) && !href.Contains("archive") && !href.Contains("search") && !href.Contains("/english") &&
                !href.Equals("/news"))
                    {

                        categoryLinks.Add("https://www.thedailystar.net" + href);
                    }
                }
            }

            return categoryLinks.ToList();
        }
        ...
        public List<string> ExtractNewsLinks(string categoryUrl)
        {
            ...
            //var newsNodes = document.DocumentNode.SelectNodes("//div[@class='card-content pt-20 pb-20 pr-20']/h3[@class='title fs-20']/a[@href]");
            //var newsNodes = document.DocumentNode.SelectNodes("//div[contains(@class, 'card-content pt-20 pb-20 pr-20')]//a[@href]");
            var newsNodes = document.DocumentNode.SelectNodes("//div[contains(@class, 'card-content pt-20 pb-20 pr-20') or contains(@class, 'card-content card-content columns')]//a[@href]");

            var newsLinks = new HashSet<string>();
            ...
            if (newsNodes != null)
            {
                foreach (var node in newsNodes)
                {
                    ...
                    {
                        newsLinks.Add(href.StartsWith("http") ? href : "https://www.thedailystar.net" + href);
                    }
                }
            }

            return newsLinks.ToList();
        }
